...
```bash
opensoul tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `opensoul tui --deliver`
...
- Check the Gateway logs: `opensoul logs --follow`.
- Confirm the agent can run: `opensoul status` and `opensoul models status`.
...
- No agents in picker: check `opensoul agents list` and your routing config.
