...
read_when:
  - You want to use Vercel AI Gateway with OpenSoul
...
```bash
opensoul onboard --auth-choice ai-gateway-api-key
```
...
is available to that process (for example, in `~/.opensoul/.env` or via
`env.shellEnv`).
