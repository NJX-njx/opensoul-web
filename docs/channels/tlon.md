...
Tlon is a decentralized messenger built on Urbit. OpenSoul connects to your Urbit ship and can
...
```bash
opensoul plugins install @opensoul/tlon
```

Local checkout (when running from a git repo):

```bash
opensoul plugins install ./extensions/tlon
```
...
Use these with `opensoul message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OpenSoul replies in-thread.
