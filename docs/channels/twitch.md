...
read_when:
  - Setting up Twitch chat integration for OpenSoul
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OpenSoul connects as a Twitch user (bot account) to receive and send messages in channels.
...
4. Configure the token:
   - Env: `OPENSOUL_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "opensoul", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OPENSOUL_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPENSOUL_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
