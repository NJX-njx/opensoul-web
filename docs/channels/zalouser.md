...
read_when:
  - Setting up Zalo Personal for OpenSoul
...
- Install via CLI: `opensoul plugins install @opensoul/zalouser`
- Or from a source checkout: `opensoul plugins install ./extensions/zalouser`
...
- `opensoul pairing list zalouser`
- `opensoul pairing approve zalouser <code>`
...
- On startup, OpenSoul resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- Re-login: `opensoul channels logout --channel zalouser && opensoul channels login --channel zalouser`
