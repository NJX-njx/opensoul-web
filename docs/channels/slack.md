...
3. Set tokens for OpenSoul and start the gateway.
...
7. Slash Commands → create `/opensoul` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
OpenSoul can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, OpenSoul falls back to the user token.
...
```json
{
  "display_information": {
    "name": "OpenSoul",
    "description": "Slack connector for OpenSoul"
  },
  "features": {
    "bot_user": {
      "display_name": "OpenSoul",
      "always_online": false
    },
    "app_home": {
...
By default, OpenSoul replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, OpenSoul infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `opensoul pairing approve slack <code>`.
...
- On startup, OpenSoul resolves channel/user names in allowlists to IDs (when tokens allow)
