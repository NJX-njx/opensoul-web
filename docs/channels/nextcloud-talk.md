...
```bash
opensoul plugins install @opensoul/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
opensoul plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OpenSoul" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `opensoul pairing list nextcloud-talk`
  - `opensoul pairing approve nextcloud-talk <CODE>`
