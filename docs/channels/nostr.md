...
read_when:
  - You want OpenSoul to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables OpenSoul to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`opensoul onboard`) and `opensoul channels add` list optional channel plugins.
...
```bash
opensoul plugins install --link <path-to-opensoul>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "opensoul@example.com",
        "lud16": "opensoul@example.com"
      }
    }
  }
}
