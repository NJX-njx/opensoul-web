...
Matrix is an open, decentralized messaging protocol. OpenSoul connects as a Matrix **user**
...
```bash
opensoul plugins install @opensoul/matrix
```

Local checkout (when running from a git repo):

```bash
opensoul plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `opensoul plugins install @opensoul/matrix`
   - From a local checkout: `opensoul plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OpenSoul calls the same
     login endpoint, stores the access token in `~/.opensoul/credentials/matrix/credentials.json`,
...
- On first connection, OpenSoul requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OpenSoul logs a warning.
...
`~/.opensoul/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, OpenSoul resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
