...
`opensoul doctor` is the repair + migration tool for OpenSoul. It fixes stale
...
- Extra workspace dir detection (`~/opensoul`).
...
you to run `opensoul doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.opensoul/opensoul.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.opensoul/sessions/` to `~/.opensoul/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.opensoul/agent/` to `~/.opensoul/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.opensoul/credentials/*.json` (except `oauth.json`)
  - to `~/.opensoul/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `opensoul doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.opensoul` folders exist across
  home directories or when `OPENSOUL_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.opensoul/opensoul.json` is
...
offers to remove them and install the OpenSoul service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named OpenSoul gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `opensoul doctor --generate-gateway-token` to force token
...
- `opensoul doctor` prompts before rewriting supervisor config.
- `opensoul doctor --yes` accepts the default repair prompts.
- `opensoul doctor --repair` applies recommended fixes without prompts.
- `opensoul doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `opensoul gateway install --force`.
