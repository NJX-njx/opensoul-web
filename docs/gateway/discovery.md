...
- TXT keys (non-secret):
  ...
  - `canvasPort=18793` (default canvas host port; serves `/__opensoul__/canvas/`)
  - `cliPath=<path>` (optional; absolute path to a runnable `opensoul` entrypoint or binary)
  - `tailnetDns=<magicdns>` (optional hint; auto-detected when Tailscale is available)

Disable/override:

- `OPENSOUL_DISABLE_BONJOUR=1` disables advertising.
- `gateway.bind` in `~/.opensoul/opensoul.json` controls the Gateway bind mode.
- `OPENSOUL_SSH_PORT` overrides the SSH port advertised in TXT (defaults to 22).
- `OPENSOUL_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS).
- `OPENSOUL_CLI_PATH` overrides the advertised CLI path.
