...
- `opensoul gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `opensoul doctor` reports no blocking config/service issues.
- `opensoul channels status --probe` shows connected/ready channels.
...
opensoul system heartbeat last
opensoul logs --follow
```
...
opensoul approvals get --node <idOrNameOrIp>
opensoul logs --follow
opensoul status
```
...
opensoul browser start --browser-profile opensoul
...
```bash
opensoul gateway status
opensoul config get gateway.mode
opensoul config get gateway.remote.url
opensoul config get gateway.auth.mode
```
...
```bash
opensoul config get gateway.bind
opensoul config get gateway.auth.token
opensoul gateway status
opensoul logs --follow
```
