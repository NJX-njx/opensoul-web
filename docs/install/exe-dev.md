---
summary: "Run OpenSoul Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Shelley, [exe.dev](https://exe.dev)'s agent, can install OpenSoul instantly with our
...
token from `gateway.auth.token` on the VM (retrieve with `opensoul config get gateway.auth.token`, or generate one
with `opensoul doctor --generate-gateway-token`). Approve devices with `opensoul devices list` and
`opensoul devices approve <requestId>`. When in doubt, use Shelley from your browser!
