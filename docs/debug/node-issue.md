...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OpenSoul
...
Running OpenSoul via Node with `tsx` fails at startup with:

```
[opensoul] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `opensoul status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch opensoul.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node opensoul.mjs status` works on Node 25.
