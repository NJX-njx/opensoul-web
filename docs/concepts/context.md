...
read_when:
  - You want to understand what “context” means in OpenSoul
...
“Context” is **everything OpenSoul sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (OpenSoul-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How OpenSoul builds the system prompt

The system prompt is **OpenSoul-owned** and rebuilt each run. It includes:
...
By default, OpenSoul injects a fixed set of workspace files (if present):
