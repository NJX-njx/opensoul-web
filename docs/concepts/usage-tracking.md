...
- `/usage cost` in chats: local cost summary aggregated from OpenSoul session logs.
- CLI: `opensoul status --usage` prints a full per-provider breakdown.
- CLI: `opensoul channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
