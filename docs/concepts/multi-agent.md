...
- **Session store** (chat history + routing state) under `~/.opensoul/agents/<agentId>/sessions`.
...
available from `~/.opensoul/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
- State dir: `~/.opensoul` (or `OPENSOUL_STATE_DIR`)
- Workspace: `~/.opensoul/workspace` (or `~/.opensoul/workspace-<agentId>`)
- Agent dir: `~/.opensoul/agents/<agentId>/agent` (or `agents.list[].agentDir`)
...
- Workspace defaults to `~/.opensoul/workspace` (or `~/.opensoul/workspace-<profile>` when `OPENSOUL_PROFILE` is set).
- State defaults to `~/.opensoul/agents/main/agent`.
