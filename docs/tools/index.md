---
summary: "Agent tool surface for OpenSoul (browser, canvas, nodes, message, cron) replacing legacy `opensoul-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `opensoul-*` skills
title: "Tools"
---

# Tools (OpenSoul)

OpenSoul exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `opensoul-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `opensoul.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OpenSoul logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`opensoul node run`).
...
- Requires a Brave API key (recommended: `opensoul configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated OpenSoul-managed browser.
...
- Quick smoke: `opensoul nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `opensoul gateway` restart in-place)
