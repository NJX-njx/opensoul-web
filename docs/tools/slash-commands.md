...
- `commands.config` (default `false`) enables `/config` (reads/writes `opensoul.json`).
...
- For full provider usage breakdown, use `opensoul status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from OpenSoul session logs.
...
```
/debug show
/debug set messages.responsePrefix="[opensoul]"
...
- Overrides apply immediately to new config reads, but do **not** write to `opensoul.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`opensoul.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[opensoul]"
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/opensoul`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
