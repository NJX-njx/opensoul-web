...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `opensoul sandbox explain`.
...
Legacy `agent.*` configs are migrated by `opensoul doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=opensoul-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENSOUL_STATE_DIR:-$HOME/.opensoul}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
