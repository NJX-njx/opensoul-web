...
- **gateway host** → `opensoul` process on the gateway machine
...
`~/.opensoul/exec-approvals.json` directly.

CLI: `opensoul approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
