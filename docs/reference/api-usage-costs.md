...
OpenSoul features that can generate provider usage or paid API calls.
...
- `opensoul status --usage` and `opensoul channels list` show provider **usage windows**
...
`opensoul models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
