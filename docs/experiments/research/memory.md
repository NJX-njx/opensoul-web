...
read_when:
  - Designing workspace memory (~/.opensoul/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OpenSoul integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.opensoul/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.opensoul/workspace` as canonical human-readable memory.
...
```
~/.opensoul/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in OpenSoul**, but keep a separable core library.

### Why integrate into OpenSoul?

- OpenSoul already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `opensoul memory recall "…" --k 25 --since 30d`
  - `opensoul memory reflect --since 7d`
...
Pragmatic take for `~/.opensoul/workspace`:
