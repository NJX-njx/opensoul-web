...
opensoul system heartbeat last
opensoul logs --follow
opensoul config get agents.defaults.heartbeat
opensoul channels status --probe
```
...
```bash
opensoul config get agents.defaults.heartbeat.activeHours
opensoul config get agents.defaults.heartbeat.activeHours.timezone
opensoul config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
opensoul cron list
opensoul logs --follow
```
