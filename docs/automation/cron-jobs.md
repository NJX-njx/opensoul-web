...
- Jobs persist under `~/.opensoul/cron/` so restarts don’t lose schedules.
...
opensoul cron list
opensoul cron run <job-id>
opensoul cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.opensoul/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `opensoul cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, OpenSoul defaults to `announce`.
...
- Job store: `~/.opensoul/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.opensoul/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
opensoul cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
opensoul cron edit <jobId> --agent ops
opensoul cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
opensoul cron run <jobId>
opensoul cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
opensoul cron edit <jobId> \
...
```bash
opensoul system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`opensoul system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `OPENSOUL_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- OpenSoul applies exponential retry backoff for recurring jobs after consecutive errors:
