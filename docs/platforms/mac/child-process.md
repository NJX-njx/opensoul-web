...
- The app installs a per‑user LaunchAgent labeled `ai.opensoul.gateway`
  (or `ai.opensoul.<profile>` when using `--profile`/`OPENSOUL_PROFILE`; legacy `com.opensoul.*` is supported).
