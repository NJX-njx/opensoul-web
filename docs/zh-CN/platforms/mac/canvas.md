...
- `~/Library/Application Support/OpenSoul/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `opensoul-canvas://<session>/<path>`

示例：

- `opensoul-canvas://main/` → `<canvasRoot>/main/index.html`
- `opensoul-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `opensoul-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
opensoul nodes canvas present --node <id>
opensoul nodes canvas navigate --node <id> --url "/"
opensoul nodes canvas eval --node <id> --js "document.title"
opensoul nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__opensoul__/a2ui/
```
...
opensoul nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
opensoul nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "opensoul://agent?message=Review%20this%20design";
```
