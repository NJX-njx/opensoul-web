...
安装文档：[安装](/install/index)、[安装程序标志](/install/installer)、[更新](/install/updating)。
...
参阅[仪表板](/web/dashboard)和 [Web 界面](/web/index)了解绑定模式和认证详情。
...
提示：小型 Pi/VPS 可以托管 Gateway 网关，你可以在笔记本/手机上配对**节点**以获取本地屏幕/摄像头/画布或命令执行能力。参阅[节点](/nodes/index)。
...
文档：[Linux](/platforms/linux)、[安装](/install/index)。
...
更多详情：[安装](/install/index)和[安装程序标志](/install/installer)。
...
中心：[平台](/platforms/index)。远程访问：[Gateway 网关远程](/gateway/remote)。
节点：[节点](/nodes/index)、[节点 CLI](/cli/nodes)。
...
通常不适合。OpenSoul 需要大上下文 + 强安全性；小显卡会截断且泄漏。如果必须使用，请在本地运行你能运行的**最大** MiniMax M2.1 版本（LM Studio），参阅 [/gateway/local-models](/gateway/local-models)。较小/量化的模型会增加提示注入风险——参阅[安全](/gateway/security/index)。
...
文档：[iMessage](/channels/imessage)、[节点](/nodes/index)、[Mac 远程模式](/platforms/mac/remote)。
...
文档：[节点](/nodes/index)、[节点 CLI](/cli/nodes)。
...
如果你想要两全其美，将 Gateway 网关保持在专用主机上，并将笔记本配对为**节点**以获取本地屏幕/摄像头/执行工具。参阅[节点](/nodes/index)。
安全指南请阅读[安全](/gateway/security/index)。
...
文档：[Gateway 网关](/gateway/index)、[渠道](/channels/index)、[多智能体](/concepts/multi-agent)、
[记忆](/concepts/memory)。
...
文档：[节点](/nodes/index)、[远程访问](/gateway/remote)、[多智能体路由](/concepts/multi-agent)、[子智能体](/tools/subagents)、[TUI](/web/tui)。
...
安全提醒：配对 macOS 节点允许在该机器上执行 `system.run`。只配对你信任的设备，并查阅[安全](/gateway/security/index)。

文档：[节点](/nodes/index)、[Gateway 网关协议](/gateway/protocol)、[macOS 远程模式](/platforms/mac/remote)、[安全](/gateway/security/index)。
...
文档：[Tailscale](/gateway/tailscale)、[远程访问](/gateway/remote)、[渠道](/channels/index)。
...
文档：[节点](/nodes/index)、[节点 CLI](/cli/nodes)、[Chrome 扩展](/tools/chrome-extension)。
...
文档：[节点](/nodes/index)、[节点 CLI](/cli/nodes)、[多 Gateway 网关](/gateway/multiple-gateways)。
...
[浏览器](/tools/browser)、[Chrome 扩展](/tools/chrome-extension)、[节点](/nodes/index)。
...
重要警告：较弱/过度量化的模型更容易受到提示注入和不安全行为的影响。参阅[安全](/gateway/security/index)。
...
[模型提供商](/concepts/model-providers)、[安全](/gateway/security/index)、
...
如果你手动运行 Gateway 网关，`opensoul gateway --force` 可以回收端口。参阅 [Gateway 网关](/gateway/index)。
...
文档：[Windows (WSL2)](/platforms/windows)、[Gateway 网关服务运维手册](/gateway/index)。
...
文档：[渠道](/channels/index)、[故障排除](/gateway/troubleshooting)、[远程访问](/gateway/remote)。
...
文档：[Gateway 网关服务运维手册](/gateway/index)。
...
文档：[安全](/gateway/security/index)、[配对](/channels/pairing)。
...
如果你想实验，在专用账户上进行并保持隔离。参阅[安全](/gateway/security/index)。

### 个人助理任务可以使用更便宜的模型吗

可以，**如果**智能体仅用于聊天且输入是可信的。较小的模型更容易受到指令劫持，因此避免将它们用于启用工具的智能体或读取不可信内容时。如果你必须使用较小的模型，锁定工具并在沙箱中运行。参阅[安全](/gateway/security/index)。
...
文档：[思考和详细输出](/tools/thinking)、[安全](/gateway/security/index#reasoning--verbose-output-in-groups)。
