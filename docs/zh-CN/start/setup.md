...
- **旧版 OAuth 导入**：`~/.opensoul/credentials/oauth.json`
  更多详情：[安全](/gateway/security/index#credential-storage-map)。
...
对于常驻或多用户服务器，考虑使用**系统**服务而不是用户服务（不需要 lingering）。参阅 [Gateway 网关运行手册](/gateway/index) 了解 systemd 说明。

## 相关文档

- [Gateway 网关运行手册](/gateway/index)（标志、监督、端口）
