---
read_when:
  - 你想在 OpenSoul 中使用注重隐私的推理服务
  - 你需要 Venice AI 设置指导
summary: 在 OpenSoul 中使用 Venice AI 注重隐私的模型
title: Venice AI
x-i18n:
...
```bash
opensoul onboard --auth-choice venice-api-key
```
...
```bash
opensoul chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## 模型选择

设置完成后，OpenSoul 会显示所有可用的 Venice 模型。根据你的需求选择：
...
opensoul models set venice/llama-3.3-70b
```

列出所有可用模型：

```bash
opensoul models list | grep venice
```
...
当设置了 `VENICE_API_KEY` 时，OpenSoul 会自动从 Venice API 发现模型。如果 API 不可达，则回退到静态目录。
...
# 使用编程模型
opensoul chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
Venice 模型目录会动态更新。运行 `opensoul models list` 查看当前可用的模型。部分模型可能暂时离线。
