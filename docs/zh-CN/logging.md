...
你可以在 `~/.opensoul/opensoul.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/opensoul.log"
  }
}
```
...
```bash
opensoul channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.opensoul/opensoul.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/opensoul/opensoul-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- OpenSoul 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
OPENSOUL_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `opensoul plugins enable diagnostics-otel` 启用插件。
...
- `opensoul.tokens`（计数器，属性：`opensoul.token`、`opensoul.channel`、`opensoul.provider`、`opensoul.model`）
- `opensoul.cost.usd`（计数器，属性：`opensoul.channel`、`opensoul.provider`、`opensoul.model`）
- `opensoul.run.duration_ms`（直方图，属性：`opensoul.channel`、`opensoul.provider`、`opensoul.model`）
- `opensoul.context.tokens`（直方图，属性：`opensoul.context`、`opensoul.channel`、`opensoul.provider`、`opensoul.model`）

消息流：

- `opensoul.webhook.received`（计数器，属性：`opensoul.channel`、`opensoul.webhook`）
- `opensoul.webhook.error`（计数器，属性：`opensoul.channel`、`opensoul.webhook`）
- `opensoul.webhook.duration_ms`（直方图，属性：`opensoul.channel`、`opensoul.webhook`）
- `opensoul.message.queued`（计数器，属性：`opensoul.channel`、`opensoul.source`）
- `opensoul.message.processed`（计数器，属性：`opensoul.channel`、`opensoul.outcome`）
- `opensoul.message.duration_ms`（直方图，属性：`opensoul.channel`、`opensoul.outcome`）

队列 + 会话：

- `opensoul.queue.lane.enqueue`（计数器，属性：`opensoul.lane`）
- `opensoul.queue.lane.dequeue`（计数器，属性：`opensoul.lane`）
- `opensoul.queue.depth`（直方图，属性：`opensoul.lane` 或 `opensoul.channel=heartbeat`）
- `opensoul.queue.wait_ms`（直方图，属性：`opensoul.lane`）
- `opensoul.session.state`（计数器，属性：`opensoul.state`、`opensoul.reason`）
- `opensoul.session.stuck`（计数器，属性：`opensoul.state`）
- `opensoul.session.stuck_age_ms`（直方图，属性：`opensoul.state`）
- `opensoul.run.attempt`（计数器，属性：`opensoul.attempt`）

### 导出的 span（名称 + 关键属性）

- `opensoul.model.usage`
  - `opensoul.channel`、`opensoul.provider`、`opensoul.model`
  - `opensoul.sessionKey`、`opensoul.sessionId`
  - `opensoul.tokens.*`（input/output/cache_read/cache_write/total）
- `opensoul.webhook.processed`
  - `opensoul.channel`、`opensoul.webhook`、`opensoul.chatId`
- `opensoul.webhook.error`
  - `opensoul.channel`、`opensoul.webhook`、`opensoul.chatId`、`opensoul.error`
- `opensoul.message.processed`
  - `opensoul.channel`、`opensoul.outcome`、`opensoul.chatId`、`opensoul.messageId`、`opensoul.sessionKey`、`opensoul.sessionId`、`opensoul.reason`
- `opensoul.session.stuck`
  - `opensoul.state`、`opensoul.ageMs`、`opensoul.queueDepth`、`opensoul.sessionKey`、`opensoul.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `opensoul doctor`。
