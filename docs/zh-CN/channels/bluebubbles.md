...
- localhost 信任意味着同主机的反向代理可能无意中绕过密码验证。如果你使用代理 Gateway 网关，请在代理处要求身份验证并配置 `gateway.trustedProxies`。参见 [Gateway 网关安全性](/gateway/security/index#reverse-proxy-configuration)。
...
有关通用渠道工作流参考，请参阅[渠道](/channels/index)和[插件](/tools/plugin)指南。
