...
- Publish as `opensoul/plugin-sdk` (or export from core under `opensoul/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OpenSoulPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OpenSoulConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OpenSoulConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OpenSoulConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OpenSoulConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OpenSoulConfig): string;
  };
};
```
...
- Introduce `opensoul/plugin-sdk`.
- Add `api.runtime` to `OpenSoulPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `opensoulRuntime: ">=2026.2.0"`).
